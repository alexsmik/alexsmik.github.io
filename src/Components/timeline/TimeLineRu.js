...
import timelineElementsRu from "./timelineElementsRu";
...
const TimeLine = () => {
    ...
    const { id, link, image, title, buttonText, date, description, icon } = timelineElementsRu[index];

    const checkNumber = (number) => {
        if (number > timelineElementsRu.length - 1) {
            return 0;
        }
        if (number < 0) {
            return timelineElementsRu.length - 1;
        }
        return number;
    };
    const nextElement = (e) => {
    ...
    return (
        <>
            <VerticalTimeline>
                {timelineElementsRu.map((element) => {
                    // let {} = element;
